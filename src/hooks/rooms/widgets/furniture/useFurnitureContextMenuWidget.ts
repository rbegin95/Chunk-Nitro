...
import { CreateLinkEvent, GetRoomEngine, IsOwnerOfFurniture, TryJoinGroup, TryVisitRoom } from '../../../../api';
        ...
        if(name)
        {
            switch(name)
            {
                ...
                case 'go_to_group_homeroom':
                    ...
                    break;
                    case 'open_forum':
                        if (groupData?.guildId) {
                            console.log(`Opening Group Forum for Group ID: ${groupData.guildId}`);
                            CreateLinkEvent(`group-forum/open/${groupData.guildId}`);
                        }
                        break;
